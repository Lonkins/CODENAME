...
| mGBA (and its bundled dependencies) | @ `e31759b` | MPL-2.0 (see `App/CoreLicences/mgba.txt`) | Emulator core, built unpatched from pinned source by `Scripts/build-cores.sh` (CI), bundled as a separate plug-in work (ADR 0001) | https://github.com/libretro/mgba |
| Beetle PSX (mednafen_psx) | @ `ef51860` | GPL-2.0 (see `App/CoreLicences/beetle_psx.txt`) | Emulator core, built unpatched from pinned source by `Scripts/build-cores.sh` (CI), bundled **helper-only** as a separate plug-in work; corresponding source published with each release (ADR 0001, ADR 0007) | https://github.com/libretro/beetle-psx-libretro |
