...
| mGBA (and its bundled dependencies) | @ `e31759b` | MPL-2.0 (see `App/CoreLicences/mgba.txt`) | Emulator core, built unpatched from pinned source by `Scripts/build-cores.sh` (CI), bundled as a separate plug-in work; load-verified, not yet conformance-run against content | https://github.com/libretro/mgba |
